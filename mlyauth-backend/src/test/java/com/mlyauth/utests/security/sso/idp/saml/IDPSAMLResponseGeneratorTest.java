...
public class IDPSAMLResponseGeneratorTest {
    ...
    @Test
    public void given_a_saml_sp_app_when_generate_a_response_the_the_assertion_must_be_well_encrypted() throws Exception {
        given_an_application(APPLICATION_ID, new AuthAspectType[]{AuthAspectType.SP_SAML});
        given_the_application_sp_sso_url(APPLICATION_ID, SP_SAMLSSO_URL);
        given_the_application_sp_entity_id(APPLICATION_ID, IDP_ENTITY_ID);
        givent_the_application_sp_encryption_certificate(APPLICATION_ID);
    ...
    private PrivateKey loadTestingPrivateKey() throws KeyStoreException, IOException, NoSuchAlgorithmException, CertificateException, UnrecoverableEntryException {
        KeyStore ks = KeyStore.getInstance(KeyStore.getDefaultType());
        InputStream is = getClass().getResourceAsStream("/keysstores/saml/samlKS.jks");
        ks.load(is, "Bourso$17".toCharArray());
        KeyStore.PrivateKeyEntry pkEntry = (KeyStore.PrivateKeyEntry) ks.getEntry("sgi.prima-solutions.com", new KeyStore.PasswordProtection("Bourso$17".toCharArray()));
    ...
    private void and_decrypt_Assertion() throws CertificateException, KeyStoreException, IOException, NoSuchAlgorithmException, UnrecoverableEntryException, DecryptionException {
        BasicX509Credential credential = samlHelper.toBasicX509Credential(ENCRYPTION_CERTIFICATE);
        credential.setPrivateKey(loadTestingPrivateKey());
