--liquibase formatted SQL


--changeset ahmed.elidrissi:1 runOnChange:true
INSERT INTO APPLICATION (ID, TYPE, APP_NAME, TITLE)
VALUES (9000, 'POLICY', 'PolicyDev', 'Policy Dev');

INSERT INTO APPLICATION_ASPECT (APPLICATION_ID, ASPECT_CODE)
VALUES (9000, 'SP_BASIC');

INSERT INTO APPLICATION_ASPECT (APPLICATION_ID, ASPECT_CODE)
VALUES (9000, 'SP_SAML');

INSERT INTO APPLICATION_ASPECT_ATTR (APPLICATION_ID, ASPECT_CODE, ATTRIBUTE_CODE, ATTRIBUTE_VALUE)
VALUES (9000, 'SP_SAML', 'Auth:SP:SAML:EndPoint', 'http://localhost:8889/primainsure/S/S/O/saml/SSO');

INSERT INTO APPLICATION_ASPECT_ATTR (APPLICATION_ID, ASPECT_CODE, ATTRIBUTE_CODE, ATTRIBUTE_VALUE)
VALUES (9000, 'SP_SAML', 'Auth:SP:SAML:Entity:ID', 'primainsure4sgi');

INSERT INTO APPLICATION_ASPECT_ATTR (APPLICATION_ID, ASPECT_CODE, ATTRIBUTE_CODE, ATTRIBUTE_VALUE)
VALUES (9000, 'SP_SAML', 'Auth:SP:SAML:Encryption:Certificate',
'
YlSjs+QT20el');


INSERT INTO AUTHENTICATION_INFO (ID, LOGIN, PASSWORD, STATUS, EFFECTIVE_AT, EXPIRES_AT)
VALUES (9901, 'cl.prima.client.dev', '$2a$04$9.FHA4Y1xJsMdGvCqWlCJ.k0MbZd/uQ3hcfqvIop7TSfRYtncsM5y', 'ACTIVE',
'2018-01-14 18:28:39', '2222-02-14 18:28:56');

INSERT INTO AUTHENTICATION_INFO_BY_LOGIN (ID, AUTHENTICATION_INFO_ID, LOGIN)
VALUES (9901, '9901', 'cl.prima.client.dev');

INSERT INTO APPLICATION (ID, TYPE, APP_NAME, TITLE, AUTHENTICATION_INFO_ID)
VALUES (9901, 'STORE', 'primaClientDev', 'Prima Client Dev', 9901);

INSERT INTO APPLICATION_ASPECT (APPLICATION_ID, ASPECT_CODE)
VALUES (9901, 'CL_JOSE');

INSERT INTO APPLICATION_ASPECT_ATTR (APPLICATION_ID, ASPECT_CODE, ATTRIBUTE_CODE, ATTRIBUTE_VALUE)
VALUES (9901, 'CL_JOSE', 'Auth:CL:JOSE:Context', 'http://localhost');

INSERT INTO APPLICATION_ASPECT_ATTR (APPLICATION_ID, ASPECT_CODE, ATTRIBUTE_CODE, ATTRIBUTE_VALUE)
VALUES (9901, 'CL_JOSE', 'Auth:CL:JOSE:Entity:ID', 'prima.client.dev');

INSERT INTO APPLICATION_ASPECT_ATTR (APPLICATION_ID, ASPECT_CODE, ATTRIBUTE_CODE, ATTRIBUTE_VALUE)
VALUES (9901, 'CL_JOSE', 'Auth:CL:JOSE:Encryption:Certificate',
'');

INSERT INTO TOKEN (ID, VALIDATION_MODE, REFRESH_MODE, STAMP, CHECKSUM, TYPE, NORM, PURPOSE, ISSUANCE_TIME,
EFFECTIVE_TIME, EXPIRY_TIME, STATUS, APPLICATION_ID, AUTHENTICATION_SESSION_ID)
VALUES (9901, 'STRICT', 'EACH_TIME', '79448893b8e5c524065f5b93ae5b4f2a57a1998feb8c008064437792dd9c9734',
'5452f4f049aafaabaab76203d94d576b55fc4ed63604a9f908f7a09c403d7f5a', 'REFRESH', 'JOSE', 'DELEGATION',
'2018-03-18 14:06:02', '2018-03-18 14:07:27', '2021-03-18 14:07:32', 'READY', 9901, null);

INSERT INTO TOKEN_CLAIM (ID, CODE, VALUE, TOKEN_ID)
VALUES (9901, 'audience', 'prima.client.dev', 9901);