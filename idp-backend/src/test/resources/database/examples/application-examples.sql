--liquibase formatted SQL
--changeset mlyahmed:1 runOnChange:FALSE
DELETE FROM APPLICATION_ASPECT_ATTR WHERE APPLICATION_ID IN (9000);
DELETE FROM APPLICATION_ASPECT WHERE APPLICATION_ID IN (9000);
DELETE FROM PERSON_APPLICATION WHERE APPLICATION_ID In (9000);
DELETE FROM APPLICATION WHERE ID IN (9000);

INSERT INTO APPLICATION (ID, APP_NAME, TITLE) VALUES (9000, 'PolicyDev', 'Policy Dev');

INSERT INTO APPLICATION_ASPECT (APPLICATION_ID, ASPECT_CODE) VALUES (9000, 'SP_BASIC');

INSERT INTO APPLICATION_ASPECT (APPLICATION_ID, ASPECT_CODE) VALUES (9000, 'SP_SAML');
INSERT INTO APPLICATION_ASPECT_ATTR (APPLICATION_ID, ASPECT_CODE, ATTRIBUTE_CODE, ATTRIBUTE_VALUE) VALUES (9000, 'SP_SAML', 'Auth:SP:SAML:EndPoint', 'http://localhost:8889/primainsure/S/S/O/saml/SSO');
INSERT INTO APPLICATION_ASPECT_ATTR (APPLICATION_ID, ASPECT_CODE, ATTRIBUTE_CODE, ATTRIBUTE_VALUE) VALUES (9000, 'SP_SAML', 'Auth:SP:SAML:Entity:ID', 'primainsure4sgi');
INSERT INTO APPLICATION_ASPECT_ATTR (APPLICATION_ID, ASPECT_CODE, ATTRIBUTE_CODE, ATTRIBUTE_VALUE) VALUES (9000, 'SP_SAML', 'Auth:SP:SAML:Encryption:Certificate',
'
YlSjs+QT20el');