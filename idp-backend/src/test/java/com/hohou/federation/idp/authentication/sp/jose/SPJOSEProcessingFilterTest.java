...
import com.hohou.federation.idp.context.mocks.MockContext;
import com.hohou.federation.idp.credentials.CredentialsPair;
import com.hohou.federation.idp.credentials.mocks.MockCredentialManager;
...
import com.hohou.federation.idp.token.jose.JOSEHelper;
import com.hohou.federation.idp.token.jose.JOSETokenDecoderImpl;
import com.hohou.federation.idp.token.jose.JOSETokenFactoryImpl;
import com.hohou.federation.idp.token.jose.mocks.MockJOSEAccessTokenValidator;
...
public class SPJOSEProcessingFilterTest {
    ...
    private void set_up_filter() {
        ...
        ReflectionTestUtils.setField(tokenDecoder, "tokenFactory", new JOSETokenFactoryImpl());
        ReflectionTestUtils.setField(tokenDecoder, "joseHelper", new JOSEHelper());
        setField(tokenDecoder, "credentialManager", credentialManager);


        filter = new SPJOSEProcessingFilter();
        ReflectionTestUtils.setField(filter, "accessTokenValidator", new MockJOSEAccessTokenValidator(true));
        setField(filter, "authenticationManager", authenticationManager);
        setField(filter, "tokenMapper", tokenMapper);
        ReflectionTestUtils.setField(filter, "context", new MockContext());
    ...
    private void given_the_scopes_list_is_not_person() {
        token.setIssuer(PEER_IDP_ID);
        token.setScopes(new HashSet<>(singletonList(TokenScope.POLICY)));
        token.setBP("SSO");
        token.setTargetURL("https://sp.prima-idp.com/sp/jose/sso");
        token.cypher();
    }
