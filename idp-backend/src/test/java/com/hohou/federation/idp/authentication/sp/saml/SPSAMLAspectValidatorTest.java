...
public class SPSAMLAspectValidatorTest {
    ...
    public void setup() {
        ...
        application = Application.newInstance().setId(APPLICATION_ID).setAspects(new HashSet<>(Arrays.asList(AspectType.SP_SAML)));
        when(appAspectAttrDAO.findByAppAndAspect(APPLICATION_ID, AspectType.SP_SAML.name())).thenReturn(attributes);
    }
    ...
    @Test(expected = MissingSPSAMLAspectAttributeExc.class)
    ...
    @Test(expected = BadSPSAMLAspectAttributeValueExc.class)
