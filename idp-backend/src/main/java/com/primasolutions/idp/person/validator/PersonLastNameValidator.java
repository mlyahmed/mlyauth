...
public class PersonLastNameValidator {
    ...
    public void validate(final String lastName) {
        if (StringUtils.isEmpty(lastName))
            throw IDPException.newInstance().setErrors(asList(AuthError.newInstance("LAST_NAME_EMPTY")));

        if (lastName.length() > MAX_LENGTH)
            throw IDPException.newInstance().setErrors(Arrays.asList(AuthError.newInstance("LAST_NAME_TOO_LONG")));
    }
