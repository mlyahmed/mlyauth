...
public class SPSAMLUserDetailsServiceImpl implements SAMLUserDetailsService {
    ...
    public Object loadUserBySAML(final SAMLCredential cred) throws UsernameNotFoundException {
        ...
        final Person person = personDAO.findByExternalId(cred.getAttributeAsString(CLIENT_ID.getValue()));
    ...
    private void checkCredentials(final SAMLCredential credential) {
        ...
        notNull(credential.getAttributeAsString(CLIENT_ID.getValue()), "The clientId attribute is missing");
        notNull(credential.getAttributeAsString(CLIENT_PROFILE.getValue()), "The profile attribute is missing");
        ...
        if (credential.getAttributeAsString(APPLICATION.getValue()) != null) {
            final boolean assigned = person.getApplications().stream().anyMatch(app -> app.getAppname()
                    .equals(credential.getAttributeAsString(APPLICATION.getValue())));
