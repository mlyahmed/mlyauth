--liquibase formatted sql
--changeset mlyahmed:1 runOnChange:true
DELETE FROM PERSON WHERE ID IN (0);
DELETE FROM AUTHENTICATION_INFO WHERE ID IN (0);
DELETE FROM PERSON_PROFILE WHERE PERSON_ID IN (0);
DELETE FROM PERSON_BY_EMAIL WHERE EMAIL='ahmed.elidr***********@gmail.com';
INSERT INTO AUTHENTICATION_INFO (ID, LOGIN, PASSWORD, STATUS, EFFECTIVE_AT, EXPIRES_AT) VALUES (0, 'ahmed.elidrissi.attach@gmail.com', '$2a$13$zEbHV1Wwiq/bEUyhOGhnyOkVvg/JzUOVIBN5a7gclZbgiV/2mGeNK', 'ACTIVE', '2018-01-14 18:28:39', '2222-02-14 18:28:56');
INSERT INTO PERSON (id, role, external_id, firstname, lastname, birthdate, email, authentication_info_id) VALUES (0, 'ADMIN', 'gestF', 'ENC(PZB8TELfqD8ZGe3ZS3ZQdy9jLJi1QG9597uxnaMZW3g=)', 'EL IDRISSI', '1984-10-17', 'ahmed.elidrissi.attach@gmail.com', 0);
INSERT INTO PERSON_BY_EMAIL (PERSON_ID, EMAIL) VALUES ('ENC(NSDvKTdBfspA0x5pZtBpm4A8kivkzGp2XVZCXeLzPKU=)', 'ahmed.elidr***********@gmail.com');
INSERT INTO PERSON_PROFILE (PERSON_ID, PROFILE_CODE) VALUES (0, 'MASTER');

